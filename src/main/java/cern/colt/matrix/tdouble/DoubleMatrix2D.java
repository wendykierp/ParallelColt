...
/**
 * Abstract base class for 2-d matrices holding <code>double</code> elements. First
...
public abstract class DoubleMatrix2D extends AbstractMatrix2D {
     ...
     * <code>a(1) == f(get(0,0)), a(0)==Double.NaN</code>.
     * <p>
     * <b>Example:</b>
     * 
     * <pre>
     ...
     * and terminators are
     * <code>a(1) == f(get(0,0),other.get(0,0)), a(0)==Double.NaN</code>.
     * <p>
     * <b>Example:</b>
     * 
     * <pre>
     ...
     * if the receiver is an instance of type <code>DenseDoubleMatrix2D</code> the
     * new matrix must also be of type <code>DenseDoubleMatrix2D</code>, if the
     * receiver is an instance of type <code>SparseDoubleMatrix2D</code> the new
     * matrix must also be of type <code>SparseDoubleMatrix2D</code>, etc. In
     ...
     * <code>DenseDoubleMatrix2D</code> the new matrix must also be of type
     * <code>DenseDoubleMatrix2D</code>, if the receiver is an instance of type
     * <code>SparseDoubleMatrix2D</code> the new matrix must also be of type
     * <code>SparseDoubleMatrix2D</code>, etc. In general, the new matrix should
     ...
     * receiver is an instance of type <code>DenseDoubleMatrix2D</code> the new
     * matrix must be of type <code>DenseDoubleMatrix1D</code>, if the receiver is
     * an instance of type <code>SparseDoubleMatrix2D</code> the new matrix must be
     * of type <code>SparseDoubleMatrix1D</code>, etc.
     * 
     * @param size
     ...
     * operations. Applies a function to a moving <code>3 x 3</code> window. Does
     * nothing if <code>rows() &lt; 3 || columns() &lt; 3</code>.
     * 
     * <pre>
     ...
     * @throws IllegalArgumentException
     *             if <code>rows() != B.rows() || columns() != B.columns()</code>.
     */
     ...
     * <code>z = alpha * A * y + beta*z</code>.
     * <code>z[i] = alpha*Sum(A[i,j] * y[j]) + beta*z[i], i=0..A.rows()-1, j=0..y.size()-1</code>
     * . Where <code>A == this</code>. <br>
     ...
     * <code>C = alpha * A x B + beta*C</code>.
     * <code>C[i,j] = alpha*Sum(A[i,k] * B[k,j]) + beta*C[i,j], k=0..n-1</code>. <br>
     * Matrix shapes: <code>A(m x n), B(n x p), C(m x p)</code>. <br>
     * Note: Matrix shape conformance is checked <i>after</i> potential
     * transpositions.
     * 
     * @param B
    ...
    /**
     * Returns the sum of all cells; <code>Sum( x[i,j] )</code>.
     * 
     * @return the sum.
     */
    public double zSum() {
     ...
     * instance of type <code>DenseDoubleMatrix2D</code> the new matrix must be of
     * type <code>DenseDoubleMatrix1D</code>, if the receiver is an instance of type
     * <code>SparseDoubleMatrix2D</code> the new matrix must be of type
     * <code>SparseDoubleMatrix1D</code>, etc.
     * 
     * @param size
     *            the number of cells the matrix shall have.
     * @param zero
